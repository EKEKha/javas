...
interface ProductInterface {
	//1. 멤버변수(=필드) : 모두 상수 (public static final 생략됨)
	//int TV = 1;
	//int COMPUTER = 2;
	//int AUDIO = 3;
	//int REFUND = 4;
...
abstract class Product {//제품-부모:"자식의 공통된 특성"
	...
	//부모의 메서드 : 자식의 공통된 기능
	abstract void show(); //미완성된 추상 메서드 : 상속한 자식클래스에서 반드시 해당 클래스에 맞게 재정의
}//Product 클래스 끝
...
class TV extends Product {
	...
	TV() {
		super(200); //부모인 Product(200) 호출 -> Product객체 생성
...
class Buyer {
	...
	void buy(Product p){
		//구매자 돈과 제품가격을 비교하여 구매자의 돈이 더 적으면 메서드 종료
		if(this.money < p.productPrice) {
			System.out.println("잔액이 부족하여 " + p +"제품을 구매할 수 없습니다.");
			return;//해당 메서드(=buy()) 종료.     ※System.exit(0);//프로그램 종료
		}
	...
	void refund(Product p) {
		if(item.isEmpty())System.out.println("구매하신 물건이 없습니다.물건을 구매해주세요.");
		
		else if (item.remove(p)) { // 제품을 ArrayList에서 제거한다.
			money += p.productPrice;    //환불받은 돈을 구매자의 돈에 추가
			bonusPoint -= p.bonusPoint;  //구매자의 보너스포인트는 감소
			System.out.println(p + "을/를 반품하셨습니다.");
		} else  // 제거에 실패한 경우
			System.out.println("구입하신 제품은 저희 매장제품이 아닙니다.");
	}
	//구매한 제품 정보 요약해서 출력
	void summary() {
		/* 구입하신 제품은 [TV, Computer]이고
		 * 구입하신 제품의 총금액은 500만원입니다.
		 * 
